...
class ProductCartUseCase @Inject constructor(private val productRepository: ProductRepository) {
    @RequiresExtension(extension = Build.VERSION_CODES.S, version = 7)
    operator fun invoke(): Flow<NetworkResponse<ProductCartModel>> = flow {
        try {
            emit(NetworkResponse.Loading())

            val response = productRepository.productCarts()

            Log.e("carts ---> ", response.body().toString())
